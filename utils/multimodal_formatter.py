...
def build_multimodal_message(input_text:str=None, 
    ...
    if input_image:
        ...
        # 定义支持的格式映射 (后缀: MIME类型)
        img_formats = {'png': 'png', 'jpg': 'jpeg', 'jpeg': 'jpeg', 'gif': 'gif', 'webp': 'webp'}
        for img_path in images:
            img_path = normalize_path(img_path)
            if not os.path.exists(img_path): continue # 跳过不存在的文件
