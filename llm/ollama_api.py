...
class OllamaAPI(BaseAPI):
    def __init__(
        ...
        self.logger.info(f"OllamaAPI - 初始化成功: {model} (Port: {port}, Context: {num_ctx})")
    ...
    def _warmup_sync(self):
        ...
        try:
            ...
            with httpx.Client() as client:
                response = client.post(self.ollama_native_url, json=payload, timeout=120.0)
                if response.status_code == 200:
                    self.logger.info(f"OllamaAPI - 模型 {self.model} 预加载成功，显存已锁定喵~")
                else:
                    self.logger.warning(f"OllamaAPI - 预加载返回异常状态码: {response.status_code}")
