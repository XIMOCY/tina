...
class Memory:
    def __init__(self,name,personality:str=None):
        ...
        self.conn.commit()
        self.checkPersonalitySet(personality)
        self.conn.close()
        self.prompt="""
    ...
    def checkPersonalitySet(self, personality):
        if self.getPersonality() is None and personality is not None:
            self.__insertInQOLite({"role":"system","tag":"personality","content":personality,"importance":5},time=datetime.datetime.now().strftime("%Y年-%m月-%d日 %H时:%M分"))
        else:
            raise ValueError("Personality没有被设置")

    async def remember(self, LLM:type,message:str) -> dict:
        """
        记忆用户信息
        importance: 1-5 重要程度
        """
        self.conn = sqlite3.connect(os.path.join(self.folder, f"{self.name}.db"))
        self.cursor = self.conn.cursor()
    ...
    def getPersonality(self)->str:
        ...
        self.cursor = self.conn.cursor()
        self.cursor.execute(
            '''SELECT content FROM logs WHERE tag = "personality"'''
        )
        ...
        if result:
            ...
            return None
    def renewPersonality(self,personality:str):
        self.conn = sqlite3.connect(os.path.join(self.folder, f"{self.name}.db"))
        self.cursor = self.conn.cursor()
        self.cursor.execute(
            '''UPDATE logs SET content =? WHERE tag = "personality"''',
            (personality,)
        )
        self.conn.commit()
        self.conn.close()
