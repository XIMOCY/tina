...
class ToolsExecutor:
            ...
            try:
                ...
                with self.sync_semaphore:
                    if _tool_name.startswith("mcp_"):
                        ...
                        # 同步模式下的核心执行与超时
                        result = self._execute_sync_logic(_tool_name, _tool_args, _tool, timeout, active_events, _tools)
                
                # After 钩子
                _, _, result = active_events.trigger_after_tool_call(_tool_name, _tool_args, result)
            except Exception as e:
                logger.error(f"ToolsExecutor - 同步执行异常: {str(e)}")
                result = f"工具执行失败: {str(e)}"
            
            _tool_calls_result[idx] = self._tool_call_result(result, _tool_id, _tool_name)

        threads = [threading.Thread(target=_worker, args=(idx, call)) for idx, call in indexed_calls]
    ...
    async def aexecute(self, _tool_calls, _tools, _mcp_client=None, timeout=60, events: AgentEvents=None, **kwargs) -> any:
        ...
        async def _async_worker(idx, tool_call):
            ...
            try:
                _tool_args = json.loads(_tool_args_raw)
                # 强制触发，None 则 Crash
                _tool_name, _tool_args = await active_events.atrigger_before_tool_call(tool_name=_tool_name, tool_arguments=_tool_args)
                ...
                _, _, result = await active_events.atrigger_after_tool_call(_tool_name, _tool_args, result)
            except Exception as e:
    ...
    async def _async_dispatch(self, _tool_name, _tool_args, _tool, timeout):
        ...
        try:
            return await asyncio.wait_for(
                loop.run_in_executor(None, lambda: self._raw_execute(_tool, _tool_args)),
                timeout=timeout
            )
    ...
    def _raw_execute(self, _tool, _tool_args):
        ...
        try:
            ...
            if tool_result is not None:
                if full_output.strip() and str(tool_result).strip() != full_output.strip():
    ...
    def _execute_sync_logic(self, _tool_name, _tool_args, _tool, timeout, active_events, _tools):
        """同步模式下的逻辑包装（含确认和线程超时）"""
        if _tool is None: return f"工具 '{_tool_name}' 未找到"
    ...
    def _execute(self, _tool_name: str, _tool_args: dict, _tool: callable, _tools, timeout=60):
        # 兼容性封装
        return self._execute_sync_logic(_tool_name, _tool_args, _tool, timeout, self.events, _tools)
