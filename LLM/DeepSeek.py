...
class DeepSeek():
    def __init__(self, api_key: str = None, model: str = "deepseek-chat",base_url:str = "https://api.deepseek.com/v1",timeout:int = 60):
        """deepseek模型使用，指定model 为 deepseek-chat 为v3模型，指定为deepseek-reasoner时为推理模型"""
        self.base_url = base_url
        if api_key is None:
            try:
                self.api_key = os.environ.get("DEEPSEEK_API_KEY")
            except:
                raise ValueError("API key并没有在环境变量‘DEEPSEEK_API_KEY’中找到，要么请你设置一下，要么输入api_key参数")
        else:
            print("我们建议你在环境变量中设置DEEPSEEK_API_KEY，不要输入api_key参数哦")
    ...
    def predict(self,
        ...
        # **非流式请求**
        if not stream:
            response = httpx.post(f"{self.base_url}/chat/completions", json=payload, headers=headers, timeout=self.__timeout)
